    ...
    static func fetchStickerPacks(fromJSON dict: [String: Any], completionHandler: @escaping ([StickerPack]) -> Void) {
        queue.async {
            ...
            for pack in packs {
                ...
                let packName1: String = pack["name1"] as! String
                let packIdentifier1: String? = pack["identifier1"] as? String
                if packIdentifier1 != nil && currentIdentifiers[packIdentifier1!] == nil {
                    currentIdentifiers[packIdentifier1!] = true
                } else {
                    if let packIdentifier1 = packIdentifier1 {
                        fatalError("Missing identifier or a sticker pack already has the identifier \(packIdentifier1).")
                    }

                    fatalError("\(packName1) must have an identifier and it must be unique.")
                }
                var stickerPack1: StickerPack?
                var stickerPack2: StickerPack?

                do {
                   stickerPack1 = try StickerPack(identifier: packIdentifier!, name: packName, publisher: packPublisher, trayImageFileName: packTrayImageFileName, publisherWebsite: packPublisherWebsite, privacyPolicyWebsite: packPrivacyPolicyWebsite, licenseAgreementWebsite: packLicenseAgreementWebsite)
                    stickerPack2 = try StickerPack(identifier: packIdentifier1!, name: packName1, publisher: packPublisher, trayImageFileName: packTrayImageFileName, publisherWebsite: packPublisherWebsite, privacyPolicyWebsite: packPrivacyPolicyWebsite, licenseAgreementWebsite: packLicenseAgreementWebsite)
                ...
                let stickers1: [[String: Any]] = pack["stickers1"] as! [[String: Any]]
                ...
                for sticker in stickers1 {
                    ...
                    do {
                        try stickerPack2!.addSticker(contentsOfFile: filename, emojis: emojis)
                ...
                for sticker in stickers {
                    ...
                    let filename = sticker["image_file"] as! String
                    do {
                        try stickerPack1!.addSticker(contentsOfFile: filename, emojis: emojis)
                    } catch StickerPackError.stickersNumOutsideAllowableRange {
                        fatalError("Sticker count outside the allowable limit (\(Limits.MaxStickersPerPack) stickers per pack).")
                    } catch StickerPackError.fileNotFound {
                        fatalError("\(filename) not found.")
                ...
                stickerPacks.append(stickerPack1!)
                stickerPacks.append(stickerPack2!)
            }
